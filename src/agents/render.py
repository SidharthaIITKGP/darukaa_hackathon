"""Rendering of a diagnosis and a ranked intervention set as text.

Extracted from src.demo so the agent graph and the standalone demo emit the
same report from the same code. src.demo imports from here; nothing here
imports src.demo.

Every number rendered here comes from src.graph.propagate. Every citation
...
from src.graph.schemas import Confidence, Measurement, SiteState
...
SITE_FIELD_LABELS: dict[str, str] = {
...
def _load_citations() -> dict[str, str]:
...
# Reverse lookup, so a citation string printed in a draft can be traced back
# to the source_id it came from. The critic needs this: it decomposes rendered
# prose into claims and has to know which registered source each claim cites
# without re-deriving it from the graph.
CITATION_TO_SOURCE_ID: dict[str, str] = {
    citation: source_id for source_id, citation in CITATIONS.items()
}
...
def years(lag: tuple[float, float]) -> str:
    if lag[0] == lag[1]:
...
def reportable(ranked: RankedIntervention) -> list[tuple[str, PropagationResult]]:
...
def explanatory_path(items: list[tuple[str, PropagationResult]]) -> list[str]:
    ...
    """
    candidates = [result.paths[0].path for _, result in items if result.paths]
    if not candidates:
        return []
...
def path_edges(graph: nx.MultiDiGraph, node_path: list[str]) -> list:
    """The first edge of each hop along a node path.

    One edge per hop, matching what "Why it works" narrates and what
    _evidence_lines cites, so the passages bound to a rendered explanation
    are the passages behind the sentences actually printed.
    """
    edges = []
...
def _why_it_works(graph: nx.MultiDiGraph, node_path: list[str]) -> str:
    ...
    sentences: list[str] = []
    for edge in path_edges(graph, node_path):
        mechanism = edge.mechanism.strip()
        if mechanism and mechanism not in sentences:
            sentences.append(mechanism)

    chain = " -> ".join(label(node) for node in node_path)
    body = " ".join(sentences)
    ...
    return f"{body}\n    Causal chain to {label(node_path[-1])}: {chain}."
...
def _evidence_lines(graph: nx.MultiDiGraph, node_path: list[str]) -> list[str]:
    ...
    for source_id, role in seen:
        ...
        lines.append(f"[{role}] {CITATIONS[source_id]}")
    return lines


def _caveat_lines(
    ranked: RankedIntervention, items: list[tuple[str, PropagationResult]]
) -> list[str]:
    caveats: list[str] = []
    ...
    low_magnitude = [
        label(result.target) for _, result in items if result.magnitude_confidence == Confidence.LOW
    ]
    if low_magnitude:
        caveats.append(
    ...
    for tradeoff in ranked.tradeoffs:
        note = (
            f"Tradeoff: {label(tradeoff.positive_target)} gains {pct(tradeoff.positive_effect)} "
            f"while {label(tradeoff.negative_target)} loses {pct(abs(tradeoff.negative_effect))}. "
...
def render_recommendation(
    ...
    title, action = ACTIONS[ranked.intervention]
    items = reportable(ranked)
    ...
    primary = items[0][1]
    explanatory = explanatory_path(items)
    lines.append(f"Why it works: {_why_it_works(graph, explanatory)}")

    lines.append("Impacted metrics:")
    width = max(len(label(target)) for target, _ in items)
    for target, result in items:
        ci = f"(90% CI {pct(result.ci90[0])} to {pct(result.ci90[1])})"
        row = f"  {label(target):<{width}}  {pct(result.p50):>7}  {ci:<34}  {years(result.lag_years)}"
    ...
    if ranked.constraint_movement is None:
        lines.append(f"  (no path to {label(limiting_var)}, this site's binding constraint)")
    elif limiting_var not in {target for target, _ in items}:
        ...
        lines.append(
            f"  {label(limiting_var):<{width}}  {pct(ranked.constraint_movement):>7}  "
    ...
    lag_high = max(result.lag_years[1] for _, result in items)
    band = _horizon_band(primary.lag_years[0])
    horizon = f"Time horizon: {band} ({years(primary.lag_years)} on {label(primary.target)}"
    ...
    lines.append(
        ...
        f"(assessed on the dominant path to {label(primary.target)}; "
    ...
    if ranked.tier == 1:
        ...
        movement = f" ({pct(constraint.p50)} on it)" if constraint is not None else ""
        lines.append(
            f"Tier 1: addresses {label(limiting_var)}, this site's binding "
            f"constraint{movement}. Ranked above everything that does not, whatever the "
            f"scores."
        )
    elif ranked.limiting_factor_addressed:
        lines.append(
            f"Tier 2: has a causal path to {label(limiting_var)}, this site's binding "
        ...
        lines.append(
            ...
            f"{label(limiting_var)}, this site's binding constraint. It earns its place on "
    ...
    caveats = _caveat_lines(ranked, items)
    if caveats:
...
def observed_state_line(site: SiteState) -> str:
    observed: list[str] = []
    for field_name in site.known():
        measurement: Measurement = getattr(site, field_name)
        display = SITE_FIELD_LABELS.get(field_name, field_name.replace("_", " "))
        if measurement.value is None:
            observed.append(f"{display} {measurement.band}")
            continue
        ...
        if unit == "%":
            observed.append(f"{display} {measurement.value:g}%")
        elif unit in ("", "pH"):
            observed.append(f"{display} {measurement.value:g}")
        else:
            observed.append(f"{display} {measurement.value:g} {unit}")
    zone = site.climate_zone()
    text = f"Observed state: {', '.join(observed) if observed else 'nothing measured yet'}."
    if zone:
        text += f" Climate zone inferred as {zone.replace('_', '-')}."
    return text
...
def render_diagnosis(
    ...
    lines.append(observed_state_line(site))
    ...
    lines.append(f"Binding constraint: {label(limiting_var)}.")
    ...
    if tier_1:
        lines.append(
            ...
            f"{label(limiting_var)} is relieved, so the ranking is a gate and not a "
            f"weighting: the {len(tier_1)} interventions that move "
            f"{label(limiting_var)} by at least {TIER_1_MIN_EFFECT:.0%} in the direction "
        ...
        lines.append(
            f"No intervention in the graph addresses {label(limiting_var)} at this site by "
...
def render_sequencing(
    ranked: list[RankedIntervention], selected: list[RankedIntervention], limiting_var: str
) -> list[str]:
    ...
    deferred = [
        item
        for item in ranked[len(selected) :]
        if item.conflicts_with_limiting_factor and item.intervention in ACTIONS
    ]
    if not (conflicting or deferred):
        return []

    lines = ["SEQUENCING"]
    lines.append(
        f"The interventions below carry a tradeoff that lands on "
        f"{label(limiting_var)}, this site's binding constraint, or on a variable "
        f"upstream of it. They are not wrong here, but they must not go first: until "
        f"{label(limiting_var)} is relieved, their own downstream gains cannot be "
        f"realised either."
    )
    for item in conflicting + deferred:
        title, _ = ACTIONS[item.intervention]
    ...
    if first:
        names = ", ".join(ACTIONS[item.intervention][0] for item in first)
        lines.append(
            f"Sequence {names} first, then revisit the interventions above once "
            f"{label(limiting_var)} has improved."
        )
    lines.append("")
    return lines
...
def render_report(
    ...
    """The full text report: diagnosis, recommendations, sequencing, method."""
    lines = render_diagnosis(site, ranked, limiting_var, limiting_why)

    selected = ranked[:top_n]
    for index, item in enumerate(selected, start=1):
        lines.extend(render_recommendation(graph, index, item, limiting_var))
        lines.append("")

    lines.extend(render_sequencing(ranked, selected, limiting_var))
